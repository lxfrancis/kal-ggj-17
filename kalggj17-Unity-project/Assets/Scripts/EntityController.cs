         ...
         for (int z = 0; z < WaveTerrain.instance.size; z++) {
            ...
            if (!entityInstances[ x, z ]) {
               ...
               foreach (Entity entity in entityPrefabs) {
                  ...
                  float lastChance  = entity.chancePerAltitude.Evaluate( lastHeight );
                  float currChance  = entity.chancePerAltitude.Evaluate( height );
                  if (currChance < lastChance) { continue; }
                  float frameChange = (currChance - lastChance) / (1.0f - lastChance);

                  if (Random.value < frameChange) {
               ...
               float lastChance  = entityInstances[ x, z ].chancePerAltitude.Evaluate( lastHeight );
               float currChance  = entityInstances[ x, z ].chancePerAltitude.Evaluate( height );
